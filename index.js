...
// ===== INIT =====
if (!BOT_TOKEN || !ADMIN_ID) { console.error("BOT_TOKEN atau ADMIN_ID belum di-set"); process.exit(1); }
...
} catch(e){ console.error("Load transaksi error:",e.message); transaksi=[]; }

// ===== HELPERS =====
function saveTransaksi(){
  try{ fs.writeFileSync(TRANSAKSI_FILE,JSON.stringify(transaksi,null,2)); }
  catch(e){ console.error("Save transaksi error:",e.message); }
}
...
async function sendEmail(to, subject, text){
  try{
    ...
    const transporter = nodemailer.createTransport({
      service:"gmail", auth:{user:EMAIL_ADDRESS, pass:EMAIL_PASSWORD}
    });
  ...
  try{
    ...
    switch(q.data){
      ...
      case "riwayat":
        ...
        await bot.sendMessage(chatId,msg); break;
...
bot.onText(/\/resi (.+) (.+)/, async (msg, match)=>{
  ...
  }catch(e){console.error("Admin resi error:",e.message);}
});
